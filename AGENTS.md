...
- **SKILL.md 结构冻结，事实数字随版本同步（audit-v4 D1③ 裁决）**：`lint_claims.skill_freeze` 实际只锁两个标记——README 含「元信息冻结」注记 + SKILL.md 含「孙学 Skill v1.0」H1 子串；结构（frontmatter name/description / 章节骨架 / 触发词组）保持冻结（license 为静态字段允许），事实数字与指针（样本计数 / reference 清单 / 版本叙述 / 必读清单）允许随 `VERSION` 演化同步更新。版本号演化在 `VERSION` 文件 + `pyproject.toml` + `CHANGELOG.md`。
...
- SKILL.md 字符预算：`lint_structure` 硬上限 25,000 chars；v1.4.0 再外化为路由器后实测 4,295 chars（含 license 字段；技法/流程/禁令真源在 references/，SKILL.md 只存路由 SOP + 铁律）。
- references/ 字符预算：每个 ≤ 12,000 chars；v1.4.0 后实测 13 个 reference（8 个超百行文件已补目录块），总 64,894 chars / 21,631 tokens（est = Σ⌊chars/3⌋，与 `token_budget` 输出同款算法，audit-v4 PE-08 口径统一；`token_budget.references_total` 上限 22,000；yingxue-anatomy.md v1.3.0 曾误记字节数 7,621 为 chars，audit-v4 勘误；v1.4.0 承接 SKILL.md 迁入的命名真源附录后 5,458 chars）。
