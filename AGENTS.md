...
- SKILL.md 字符预算：`lint_structure` 硬上限 25,000 chars；v1.3.1 实测 13,118 chars（含 license 字段，留 11,882 余量）。
- references/ 字符预算：每个 ≤ 12,000 chars；audit-v5 后实测 13 个 reference（8 个超百行文件已补目录块），总 63,352 chars / 21,114 tokens（est = Σ⌊chars/3⌋，与 `token_budget` 输出同款算法，audit-v4 PE-08 口径统一；`token_budget.references_total` 上限 22,000；yingxue-anatomy.md v1.3.0 曾误记字节数 7,621 为 chars，audit-v4 勘误）。
